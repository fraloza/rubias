...
<!--        <input type="submit" value="Buscar" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only ui-state-hover"   />-->
     ...
     <?php if($new) { ?>
        ...
        <button onclick="javascript:popup('index.php?controller=<?php echo $name; ?>&action=create',860,400)">Nuevo</button>
        <?php } ?>
        <?php // } ?>
        ...
        $n = 0;
         foreach ($rows as $value) { 
          $n ++;
           ...
           if($value['Estado']==false){  $myselect .= ")'><img src='images/front.png' /></a></td>";}
