...
class asignatura_cca extends Main{    
    ...
    function insert($_P ) {
        ...
        $sql = $this->Query("sp_asign_iu_cca(0,:p1,:p2,:p3,:p4,:p5)");
    ...
    function update($_P ) {
        ...
        $sql = $this->Query("sp_asign_iu_cca(1,:p1,:p2,:p3,:p4,:p5)");
        $stmt = $this->db->prepare($sql);
    ...
    function delete($p) {
        $stmt = $this->db->prepare("DELETE FROM asignatura_cca WHERE idasignatura = :p1");
